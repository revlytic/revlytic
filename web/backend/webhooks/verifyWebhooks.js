...
const sendEmail = async (pdfPath, email) => {
...
//  "{{card_brand_name}}":extra?.data?.contractDetails?.instrument?.brand ? (extra?.data?.contractDetails?.instrument?.brand.charAt(0).toUpperCase() + formatVariableName(extra?.data?.contractDetails?.instrument?.brand.slice(1).toLowerCase())) : "",
...
// try{
//  emailContent =await  ejs.renderFile( dirPath + "/preview2.ejs", {selectedTemplate,currencySymbol:extra?.currencySymbol,templateType:"paymentFailure" ,data:extra?.data,dateConversion,check:extra?.check});
// }
// catch(error){
//   console.log("error",error)
// }
//  console.log("after emailcontent")
...
//  "{{card_brand_name}}": extra?.data?.payment_details?.payment_instrument_value?.brand ? (extra?.data?.payment_details?.payment_instrument_value?.brand.charAt(0).toUpperCase() + extra?.data?.payment_details?.payment_instrument_value?.brand.slice(1).toLowerCase()): "",
