...
const SubscriptionProductsEdit = ({
  ...
  const handleProducts = async (e) => {
    let country;
    if (
      subscription_details?.currency?.toLowerCase() !=
      storeCurrency?.toLowerCase()
    ) {
  ...
  const handleSave = async (subscriptionLine, index) => {
    let body = {
      ...
      input: {
        ...
        currentPrice:
          subscription_details?.planType == "prepaid"
            ? parseFloat(
                price *
                  (subscription_details?.billingLength /
                    subscription_details?.delivery_billingValue)
              )
            : parseFloat(price),
      },
      line: subscriptionLine,
    ...
    <div>
      ...
      >
        ...
        <div className="product-section portal-product">
          {products.length > 0 && (
            <div className="revlytic-product-listing-header manual-list">
              <h5>Product</h5>
              <h5>Price</h5>
              <h5>Quantity</h5>
              <h5>Total</h5>
          ...
          {products?.map((item, index) => (
            <div key={index}>
                    ...
                    <div className="revlytic product-name">
                      <a
                        ...
                        href={
                          ...
                          }/products/` + item?.product_id?.split("/").at(-1)
                        }
              ...
              {index == products.length - 1 &&
                subscription_details.planType && (
                  <div className="rev-subtotal">
                    <p>Subtotal:</p>
                    <p>
                      {" "}
                      {getCurrencySymbol(subscription_details.currency) +
                        (subscription_details.planType == "prepaid"
                          ? parseFloat(
                              calculateSubTotal() *
                                (parseInt(subscription_details?.billingLength) /
                                  parseInt(
                                    subscription_details?.delivery_billingValue
                                  ))
                            )?.toFixed(2)
